...
class PaginatorTestCase(BootstrapTestCase):
    ...
    def test_url_replace_param(self):
        self.assertEqual(url_replace_param("/foo/bar?baz=foo", "baz", "yohoo"),
                         "/foo/bar?baz=yohoo")
        self.assertEqual(url_replace_param("/foo/bar?baz=foo", "baz", None),
                         "/foo/bar")
        self.assertEqual(url_replace_param("/foo/bar#id", "baz", "foo"),
                         "/foo/bar?baz=foo#id")

    def bootstrap_pagination(self, page, extra=""):
        """Render bootstrap_pagination tag."""
        return self.render(f"{{% bootstrap_pagination page {extra} %}}",
                           {"page": page})

    def test_paginator(self):
        html = self.bootstrap_pagination(self.paginator.page(2),
                                         extra='url="/projects/?foo=bar"')
    ...
    def test_paginator_size(self):
        ...
        self.assertHTMLEqual(
                self.render('{% bootstrap_pagination page size="lg" %}',
                            {"page": self.paginator.page(2)}),
                (
        ...
        self.assertHTMLEqual(
                self.render('{% bootstrap_pagination page size="md" %}',
                            {"page": self.paginator.page(2)}),
                (
                    '<ul class="pagination">'
                    '<li class="page-item"><a class="page-link" href="?page=1">&laquo;</a></li>'
        ...
        with self.assertRaisesRegex(ValueError, 'Invalid value "xl" for parameter'):
            self.render('{% bootstrap_pagination page size="xl" %}',
                        {"page": self.paginator.page(2)})

    def test_paginator_justify(self):
        self.assertHTMLEqual(
                self.render('{% bootstrap_pagination page justify_content="center" %}',
                            {"page": self.paginator.page(2)}),
                (
        ...
        with self.assertRaises(ValueError):
            self.render(
                    '{{% bootstrap_pagination page justify_content="somewhere" %}',
                    {"page": self.paginator.page(2)}
                    )

    def test_paginator_illegal(self):
        with self.assertRaises(ValueError):
            self.render('{% bootstrap_pagination page pages_to_show="foo" %}',
                        {"page": self.paginator.page(2)})
        with self.assertRaises(ValueError):
            self.render("{{% bootstrap_pagination page pages_to_show=-5 %}",
                        {"page": self.paginator.page(2)})
    ...
    def test_paginator_extra_css(self):
        ...
        self.assertHTMLEqual(
                self.render('{% bootstrap_pagination page extra_classes="custom-pagination" %}',
                            {"page": self.paginator.page(2)}),
                (
        ...
        self.assertHTMLEqual(
                self.render('{% bootstrap_pagination page extra_classes="my-custom-class another-class" %}',
                            {"page": self.paginator.page(2)}),
                (
        ...
        self.assertHTMLEqual(
                self.render('{% bootstrap_pagination page extra_classes="" %}',
                            {"page": self.paginator.page(2)}),
                (
                    '<ul class="pagination">'
                    '<li class="page-item"><a class="page-link" href="?page=1">&laquo;</a></li>'
    ...
    def test_paginator_extra_classes_with_size(self):
        ...
        self.assertHTMLEqual(
                self.render('{% bootstrap_pagination page size="sm" extra_classes="custom-small" %}',
                            {"page": self.paginator.page(2)}),
                (
        ...
        self.assertHTMLEqual(
                self.render('{% bootstrap_pagination page size="lg" extra_classes="large-custom" %}',
                            {"page": self.paginator.page(2)}),
                (
    ...
    def test_paginator_extra_classes_with_justify(self):
        ...
        self.assertHTMLEqual(
                self.render('{% bootstrap_pagination page justify_content="center" extra_classes="centered-custom" %}',
                            {"page": self.paginator.page(2)}),
                (
        ...
        self.assertHTMLEqual(
                self.render('{% bootstrap_pagination page justify_content="end" extra_classes="end-aligned" %}',
                            {"page": self.paginator.page(2)}),
                (
    ...
    def test_paginator_extra_classes_with_all_options(self):
        ...
        self.assertHTMLEqual(
                self.render('{% bootstrap_pagination page size="lg" justify_content="center" '
                            'extra_classes="all-options-test" %}',
                            {"page": self.paginator.page(2)}),
                (
...
class LargePaginatorTestCase(BootstrapTestCase):
    ...
    def test_paginator_ellipsis(self):
        html = self.render("{{% bootstrap_pagination page %}",
                           {"page": self.paginator.page(33)})
    ...
    def test_paginator_ellipsis_with_extra_css(self):
        ...
        html = self.render('{% bootstrap_pagination page extra_classes="ellipsis-test" %}',
                           {"page": self.paginator.page(33)})
